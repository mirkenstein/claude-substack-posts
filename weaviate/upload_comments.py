...
Usage:
    python weaviate/upload_comments.py                                    # incremental from both DBs
    python weaviate/upload_comments.py --database substack                # substack DB only
    python weaviate/upload_comments.py --database podcasts                # podcasts DB only
...
def upload_for_database(database: str, args, client):
    """Upload comments from one database to Weaviate."""
    # Build filters
    filters = []
    params = []
    ...
    with collection.batch.fixed_size(batch_size=100) as batch:
        for row in rows:
            ...
            obj_uuid = uuid.uuid5(
                uuid.NAMESPACE_DNS,
                f"engru-comment-{database}-{row['comment_id']}"
            )
    ...
    if not args.publication:
        save_upload_time(upload_time, database)
