...
### 1. Add to substacks.json

Add the publication to `substacks.json` with its target database and ingestion mode:
...
- `database`: `substack` or `podcasts`
- `mode`: `full` (entire newsletter) or `individual` (cherry-picked articles only)

The database is auto-detected from this config throughout the pipeline — no need to pass `--database` manually. You can always override with `--database` if needed.

### 2. Set the blog variable

```bash
export BLOG=julianmacfarlane   # subdomain from https://<subdomain>.substack.com
```
...
```bash
export BLOG=corbettreport
# Resolve database from substacks.json for scripts that need it explicitly
export DB=$(python3 -c "from src.publication_config import get_database; print(get_database('${BLOG}', default='substack'))")
echo "Database: ${DB}"
...
# Step 2: Fetch full content + comments (re-run with --resume if interrupted)
python main.py --from-list posts/${BLOG}/${BLOG}_posts.json \
    --output-dir posts/${BLOG}/ --resume --delay 30 --jitter 4 -v

# Step 3: Load into PostgreSQL (database auto-detected from substacks.json)
python load_posts.py --dir posts/${BLOG}/ --resume --log etl_${BLOG}.log -v

# Step 4: Upload to Weaviate (database auto-detected from substacks.json)
python weaviate/upload_posts.py --publication ${BLOG}
python weaviate/upload_comments.py --publication ${BLOG}

# Step 5: Download media (images + audio)
python download_media.py --extract --download --publication ${BLOG} --database ${DB}

# Step 6: Analyze images
PYTHONUNBUFFERED=1 nohup python -u analyze_media.py --publication ${BLOG} --database ${DB} --model ollama/qwen3-vl:30b   2>&1|tee analyze_${BLOG}-$(date +%Y%m%d-%H%M%S).log 2>&1 &
#qwen3-vl:30b 
# Step 7: Transcribe podcasts (if applicable)
./transcribe_all.sh --subdomain ${BLOG} --database ${DB} --dry-run
./transcribe_all.sh --subdomain ${BLOG} --database ${DB}

# Step 8: Ingest transcripts into Postgres and Weaviate
python ingest_transcripts.py posts/saved/audio/${BLOG}/ --database ${DB}
```

For paid content, add `--cookies cookies.json` to Steps 1 and 2.
...
```bash
# Database auto-detected from substacks.json
python load_posts.py --dir posts/${BLOG}/ --resume --log etl_${BLOG}.log -v

# Override database manually if needed:
...
```bash
# Weaviate (database auto-detected from substacks.json)
python weaviate/upload_posts.py --publication ${BLOG}
python weaviate/upload_comments.py --publication ${BLOG}
...
# Pass 1: fast triage with local Ollama (gemma3:27b)
...
db = DatabaseConnection(database=get_database('${BLOG}'))
conn = db.connect()
cur = conn.cursor()
cur.execute('''
...
Database is auto-detected from `substacks.json` — no `--database` flag needed.

```bash
python refresh_blog.py ${BLOG}
python refresh_blog.py ${BLOG} --cookies cookies.json         # paid content
python refresh_blog.py ${BLOG} --check 20                     # check more posts
python refresh_blog.py ${BLOG} --dry-run                      # preview only
python refresh_blog.py ${BLOG} --skip-weaviate                # skip Weaviate upload
python refresh_blog.py ${BLOG} --delay 30 --jitter 4          # custom delay
```

### Batch refresh all full-ingest blogs

```bash
for BLOG in $(python3 -c "from src.publication_config import list_publications; print(' '.join(list_publications(database='substack', mode='full')))"); do
